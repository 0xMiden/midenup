...
:::important
If this message is _not_ displayed, yet this is the first time that midneup is being installed in the system, then this is probably due to an executable called "miden" already being present in the PATH; this can be the case of older VM or client releases. If that is the case, consider uninstalling those executables via `cargo uninstall miden` in order for midenup to work correctly. Nowadays, those executables are named miden-vm and miden-client respectively.
:::
