...
After those `export`s have been added to the shell's profile, restart your shell session in order for them to take effect. This will enable `midenup`'s `miden` command, which is used to interact with the different miden components.
...
Toolchains are installed via "Channels", which consist of a specific release of a toolchain with instructions on how to obtain it.
...
This command will install the specified toolchain using the [official midenup channel](https://0xmiden.github.io/midenup/channel-manifest.json).
...
## Specific releases
If required, a specific toolchain release (e.g. 0.15.0) can also be installed via:
...
# Using a toolchain
To see a quick summary of what the currently active toolchain offesrs, the `miden help toolchain` can be run.
...
There are currently 2 main mechanisms to alter this: setting a system wide default or setting a directory local default. Each method has an associated priority according to the following chart (from highest to lowest):
