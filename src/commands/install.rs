...
fn main() -> ExitCode {
    ...
    let mut exit_status = ExitCode::SUCCESS;
    {
        ...
        if !std::fs::exists(&lib_path).unwrap_or(false) {
            ...
            if should_fetch {
                println!("{padding} fetching artifact");
                if let Err(err) = install_artifact("{{ dep.artifact.0 }}", "{{ dep.artifact.1 }}") {
                    println!("{padding} error: failed to fetch artifact: {err}");
                    should_build = true;
                } else {
            ...
            if should_build {
                println!("{padding} building artifact from source");
                ...
                if let Err(err) = lib.as_ref().write_to_file(&lib_path) {
                    if {{ keep_going }} {
                        println!("{padding} error: failed to install {{ dep.name }} from source: {err}");
                    } else {
                        eprintln!("error: failed to install {{ dep.name }} from source: {err}");
            ...
            if successfully_installed {
                println!("{padding} {{ dep.name }}: installed");
            }
        } else {
            println!("{padding} already installed");
        }
        {%- endfor %}
    }
    ...
    if !std::fs::exists(&bin_path).unwrap_or(false) {
        ...
        if should_build {
            if let Err(err) = install_from_source(
                ...
                if {{ keep_going }} {
                    println!("{padding} error: failed to install {{ component.name }} from source: {err}");
                } else {
                    eprintln!("error: failed to install {{ component.name }} from source: {err}");
                    return ExitCode::FAILURE;
                }
            } else {
                successfully_installed = true;
        ...
        if successfully_installed {
            println!("{padding} {{ component.name }}: installed");
        }
    } else {
        println!("{padding} already installed");
    }
