...
pub fn latest_modification(dir: PathBuf) -> anyhow::Result<SystemTime> {
            ...
            for file in entries {
                ...
                let (current_entry_latest, visited_entries) =
                    ...
                    if metadata.is_dir() && !metadata.is_symlink() {
                        ...
                        (metadata.modified().ok(), current_entry_count + 1)
                    };

                current_entry_count = visited_entries;

                local_latest = match (local_latest, current_entry_latest) {
                    (Some(local_latest), Some(current_entry_latest)) => {
                        Some(max(current_entry_latest, local_latest))
                    },
                    (Some(local_latest), None) => Some(local_latest),
    ...
    let directory_last_modification = dir.metadata().and_then(|file| file.modified()).ok();
